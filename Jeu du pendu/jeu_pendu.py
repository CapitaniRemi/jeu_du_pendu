...
pas_valide=True #variable booleenne qui vaut Vrai si le mot n'est pas valide et faux sinon
while pas_valide:
	print("--------------\n JEU DU PENDU\n--------------\n")
...
while ca_joue:
    ...
    if lettre in alphabet and len(lettre)==1 : #on verifie que le joueur 2 donne bien une lettre
        if lettre not in proposees:              #la lettre a-t-elle été proposée
            ...
            else:                           #le lettre n'est pas dans le mot
                                                        #à compléter dans le cas où la lettre ne figure pas dans le mot
                jokers=jokers-1
            proposees+=lettre
